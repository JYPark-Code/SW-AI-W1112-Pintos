...
### 8.5 슬라이드 한 줄 풀기 — "자원 소유권의 엄격한 경계 확립 + 디버깅 시야의 확장"

발표 슬라이드에 본 audit 의 메타 교훈을 한 줄로 압축해 적었다:

> **"자원 소유권의 엄격한 경계 확립과 디버깅 시야의 확장."**

추상적으로 들리는 문장인데, 실은 이 audit 가 *어떻게 의사결정을 했는지*
의 구조를 한 줄로 묶은 것이다. 본 audit 의 디테일 (§1–§7) 을 모르는 독자도
한 번에 납득할 수 있도록 두 절반을 풀어 쓴다.

#### (1) 자원 소유권의 엄격한 경계 확립

> 모든 자원에 대해 *지금 누가 갖고 있는지, 언제 누구에게 넘어가는지, 누가
> 최종적으로 회수하는지* 가 코드에 명시적으로 박혀 있어야 한다.

본 audit 에서 발견한 누수 5개가 다 같은 모양이었다는 게 이 문장의 근거다.
어느 자원이든 코드를 따라가 보면 — **"만든 자리는 있는데, *언제 누구에게
넘어가고 결국 누가 회수하는지* 가 비어 있는"** 형태였다.

| 자원 | 만든 자리 | (이전) 회수의 짝 |
|---|---|---|
| fd 슬롯 | `SYS_OPEN` | 유저가 `SYS_CLOSE` 부르길 기다림 (안 부르면 leak) |
| 열린 `struct file` | `SYS_OPEN` | 위와 동일 |
| 이전 `running_file` (exec 시) | `load()` 가 새 파일로 덮어쓰는 순간 | 없음 — 그 자리에서 핸들이 사라짐 |
| 이전 `old_pml4` (exec 시) | `load()` 진입 시 백업 | 실패 분기에만 — 성공 분기는 비어 있음 |
| `fork_success` 신호 | `__do_fork` 가 set 했어야 | wire 자체가 없음 — 송신/수신의 짝이 비어 있었음 |

해결책의 *방향* 이 핵심이다. 다섯 자리에 *각자* 회수 코드를 박는 게 아니라
— **자원이 *반드시* 지나가는 한 자리와 *반드시* 갈아끼우는 한 자리를 찾아
거기에 회수 코드를 박는다.** 본 audit 에서는 두 자리였다:

- **`process_cleanup`** — 정상 exit, kill(-1), fork 중간 실패, exec 실패 후
  thread_exit 등 *모든 종료 경로* 가 모이는 자리. fd 와 파일을 여기서 일괄 회수.
- **`load()` 의 성공 분기** — exec 으로 새 이미지가 들어오는 *유일한 자리*. 이전
  running_file 과 old_pml4 를 여기서 명시적으로 회수.

이게 "경계 확립" 의 의미다. 자원이 *어디서 끝나는지* 가 코드의 *특정 줄* 로
정해져 있다.

"엄격한" 의 뜻은 — 회수가 *유저가 cleanup 을 부르면* 같은 조건부가 아니라,
**프로세스 종료의 모든 경로에서 무조건** 으로 박혀 있다는 것. 회수 코드를
호출할지 말지를 *부르는 쪽이 결정* 하는 게 아니라, *시스템이 보장* 한다.

#### (2) 디버깅 시야의 확장

> 버그 *한 개의 증상* 만 보지 말고, 그 버그가 *드러낸 패턴* 과 그 패턴이
> *미래의 어떤 작업과 합쳐질지* 까지 본다.

본 audit 가 *시작된 이유 자체* 가 이 교훈의 적용 사례다. `multi-oom` 이
fail 이라는 사실만 보면 작은 정보다. 좁은 시야로는 "그 테스트 하나 통과
시키자" 가 된다. 그런데 한 칸 멀리서 보면 —

> "곧 swap 을 짤 텐데, swap 은 *OOM 직전 경로에서만 실행* 되는 코드다. 그
> 위에 누수가 있으면 — *OOM 이 떴을 때 swap 버그인지 누수인지 구분 불가
> 능*. 즉 *지금의 multi-oom fail 은 미래의 swap 디버깅을 막을 신호다.*"

이 한 단락이 "시야 확장" 이다. 현재 증상의 해결이 목표가 아니라 *그 증상이
미래 작업과 어떤 관계에 있는지* 를 본 결과, 작업의 우선순위가 "테스트 한 개
통과" 에서 **"swap baseline 확보"** 로 한 단계 올라간다.

audit 진행 중에도 같은 형태의 확장이 반복됐다:

| 좁은 시야 | 시야 확장 후 |
|---|---|
| "fd_table OOB 막자" | "monotonic counter 가 고정 크기 배열에 인덱싱하는 형태는 *언젠가 반드시* 깨진다 — 일반 패턴" |
| "process_exit 에서 fd 닫자" | "모든 종료 경로가 거기로 모이니, 이 자리를 *단일 회수 지점* 패턴으로 굳히자" |
| "fork 끝의 한 줄을 고치자" | "fork 의 *성공/실패 신호 인터페이스 자체* 가 빠져 있다" |

각 자리마다 한 칸씩 멀리 봤을 때 *패턴* 이 보인다. 패턴이 보이면 *한 번의
fix 가 여러 자리에서 동시에 들음.*

#### (3) 두 절반이 *짝* 인 이유

두 절반은 별개가 아니다. 시야가 좁으면 — "여기 leak 한 군데 막자" 가 되고,
다른 자리에 같은 모양의 leak 이 있을 때 또 다른 fix 가 필요해진다.

반대로 시야가 확장돼서 *패턴* 이 보이면 — 해결책이 자연스럽게 *"모든
자리의 경계를 일관되게 박는"* 쪽으로 흐른다. 즉:

> **시야 확장이 → 소유권 경계 확립의 *방법론적 전제* 다.**

다섯 누수를 *5번의 개별 fix* 가 아니라 *1개의 패턴 fix (자원마다 명시적
회수 지점 정의)* 로 처리할 수 있었던 것은, 시야 확장이 *먼저* 있었기 때문.

```mermaid
flowchart LR
    A["디버깅 시야 확장<br/>(이 버그가<br/>드러낸 패턴은?)"]
    B["자원 소유권 경계<br/>확립<br/>(만든 자리 ↔<br/>회수 자리)"]
    C["한 번의 fix 가<br/>여러 자리에서<br/>동시에 들음"]
    A --> B --> C
    style A fill:#fff3cd,color:#000
    style B fill:#fff3cd,color:#000
    style C fill:#d4f4dd,color:#000
```

#### (4) "메타 교훈이 뭐냐" 받았을 때 — 한 단락 답변

> "이번 audit 에서 발견한 5개 버그가 다 같은 모양이었어요 — *자원을 만든
> 자리는 있는데 회수의 짝이 없는* 형태. 그래서 한 개씩 고치지 않고, 모든
> 자원에 대해 *누가 만들고 누가 회수하나* 의 짝을 명시적으로 박는 방향으로
> 정리했어요. 그게 앞의 절반 (경계 확립).
>
> 그리고 이 작업을 *왜* 했냐는 — multi-oom 한 테스트 통과시키려는 게
> 아니라, *swap 작업의 디버깅 가능성* 을 미리 확보하려는 거였어요. 버그
> 한 개가 아니라 그 버그가 *나중에 어떤 문제와 합쳐질지* 까지 봐서 결정한
> 거. 그게 뒤의 절반 (시야 확장)."

이 한 단락이 슬라이드 한 줄을 *말로* 풀어내는 답이다. §1–§7 의 디테일을
모르는 청중에게도 이 한 단락만으로 두 절반이 *왜 짝* 인지가 닫힌다.

#### (5) 안 쓰는 게 좋은 표현

같은 한 줄을 설명할 때 흔히 빠지기 쉬운 오해를 미리 막아 둔다.

- ❌ "Rust 의 ownership 처럼" — 단어가 같아도 의미가 다르다. Rust 는
  *컴파일러가 강제* 하는 것이고, 본 audit 의 "경계" 는 *코드 규약 + 단일
  회수 지점 명시* 다. 비유로 쓰면 청중이 mechanism 을 헷갈린다.
- ❌ "메모리 누수 막자" — 다섯 누수 중 *네 개* 만 메모리 관련. Bug 5
  (fork_success) 는 메모리가 아니라 *신호 wire 누락* 이다. 이 사례까지
  포함하려고 *자원* 으로 추상화한 것.
- ❌ "디버깅을 잘하자" — 너무 진부. *시야* 의 핵심은 "*시점/스코프* 의
  확장" 이다 — 현재 버그 vs 미래 작업, 한 버그 vs 패턴.

...
> | §12 | 다음날 — 남아 있던 pt 테스트 5개 추가 통과 | "두 길" 의 일관된 처리 + 자기 실수 회고 |
...
pintos/userprog/syscall.c         ← user_rsp 박제, validate_user_addr 완화,
...
`thread.h:123` 에 필드 추가, `syscall.c:81` 에서 진입 시 저장.
...
/* syscall.c, syscall_handler() 진입부 */
thread_current()->user_rsp = f->rsp;
```

#### 12.5.1 왜 필요한가

`pt-grow-stk-sc` 가 단서다. 이 테스트는 **syscall 안에서 큰 버퍼를
스택에 올린다.** 흐름:

```
유저 → SYS_READ(buf, 큰 size)
   │
   ▼ 커널 컨텍스트 진입 (cs = SEL_KCSEG)
   │
   ▼ file_read 가 buf 에 쓰는 도중 stack 페이지 부족 → page fault
   │
   ▼ vm_try_handle_fault (user = false)
       │
       └─ rsp 가 필요한데, f->rsp 는 …
              ▲
              └─ 커널이 fault 났으므로 f->rsp = 커널 스택의 rsp.
                 유저 rsp 가 아니다.
```

`f->rsp` 가 가리키는 건 **fault 시점의 rsp** 다. 유저 코드가 fault 면
`f->rsp` = 유저 rsp 가 맞지만, 커널 코드가 fault 면 `f->rsp` = 커널 rsp.
스택 확장 휴리스틱은 **유저 rsp** 가 있어야 의미가 있는데 (`addr` 도
유저 영역이니까), 커널 rsp 와 비교하면 항상 거짓이다.

#### 12.5.2 해결

syscall 진입 시점에 유저 rsp 를 thread 에 박제. 그 시점엔 아직 커널
스택으로 안 넘어갔으므로 `f->rsp` 가 유저 값. 이후 그 syscall 안에서
무슨 일이 나든 `thread_current()->user_rsp` 로 유저 rsp 를 회수할 수
있다.
...
§5.4 에서 "user 플래그를 아직 안 쓴다" 고 적어 둔 자리를 오늘 채운
셈이다.

> **메타 교훈**: `intr_frame` 의 필드는 "**fault 의 컨텍스트**" 를
> 담는다 — 그 컨텍스트가 유저인지 커널인지에 따라 같은 필드의 의미가
> 달라진다. 유저 데이터를 보존해야 하면 **유저 → 커널 경계** 에서 한 번
> 박제하는 게 정석.
...
### 12.11 자기 실수 2 — 주석 처리된 `printf` 가 `if` 의 body 를 먹은 사건

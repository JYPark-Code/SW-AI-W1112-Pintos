...
##### 잠깐 — "`buf2[65536]` 인데 32KB 지점에 접근하는 게 왜 stack growth?"

팀 회의에서 자연스럽게 나온 의문. 테스트 코드는 한 줄로 압축된다.

```c
/* tests/vm/pt-grow-stk-sc.c */
char buf2[65536];                                  /* 64KB */
read(handle, buf2 + 32768, slen);                  /* 한가운데에 read */
```

"64KB 짜리 배열 한가운데" 는 분명히 배열 *안* 인데 왜 fault 가 나고 왜
스택을 *더* 늘려야 하는가? — 다음 네 가지를 분리해서 봐야 풀린다.

###### (1) `char buf2[65536]` 이 실제로 하는 일

C 컴파일러가 이 선언을 보면 함수 진입부에 대략 한 줄을 박는다.

```
sub rsp, 65536     ; rsp 를 64KB 아래로 끌어내림
```

**이게 전부다.** 페이지를 잡거나 0 으로 채우는 일은 없다. 그냥 **스택
포인터를 64KB 아래로 이동시켰을 뿐**. 메모리 관점에서는 *아무 일도 안
일어났다.*

###### (2) rsp 와 buf2 + 32768 의 좌표

`sub rsp, 65536` 직후의 그림:

```
   USER_STACK ─────────────  ← setup_stack 으로 깔린 첫 페이지의 위쪽 끝
       │
       │  (이전 함수 호출들로 한 장씩 등록된 페이지들)
       │
   rsp_before ─────────────  ← test_main 진입 직전 rsp
       │
       │  sub rsp, 65536      ← 64KB 만큼 한 번에 점프
       │
       │     ┌─ buf2 + 65535       ← 직전 프레임 바로 아래
       │     │
       │     │   ┌─ buf2 + 32768   ← read 의 대상으로 넘긴 위치
       │     │   │
   rsp_after = buf2 ────────  ← test_main 본문 안에서의 rsp
       │
       │  (이 아래는 미래 함수 호출이 또 sub rsp 할 자리)
```

핵심 두 가지:
- **`buf2` 자체가 rsp_after 와 거의 같은 주소.** 컴파일러가 만든 새 스택
  프레임의 바닥.
- `buf2 + 32768` 은 rsp_after **위쪽으로 32KB** 떨어진 자리. fault handler
  의 §5.3 기준에서 **rsp − 8 보다 한참 위**.

###### (3) "rsp 이동" 과 "페이지 등록" 은 분리되어 있다

VM 입장에서 `sub rsp, 65536` 은 그저 한 레지스터의 값이 바뀐 사건이지
SPT 와는 무관하다. **SPT 는 누군가가 그 주소를 실제로 건드릴 때까지
모른다.**

```
sub rsp, 65536 직후의 SPT 상태:
   USER_STACK 바로 아래 1 페이지 (setup_stack 의 결과)        ✓ 등록
   + 그 사이 함수 호출들로 한 장씩 늘어난 페이지들             ✓ 등록
   buf2 + 32768 의 페이지 (= rsp + 32768 의 페이지)            ✗ 없음
   buf2 의 페이지 (= rsp 의 페이지)                            ✗ 없음
```

이게 우리 lazy 정책의 *의도된* 결과 — 페이지는 *실제로 접근될 때*만 등록.
rsp 는 즉시, 페이지는 lazy.

###### (4) 그래서 무엇이 늘어나는가

`read(handle, buf2 + 32768, slen)` 의 흐름을 따라가면:

1. 유저 코드는 *주소만* 인자로 넘긴다 — `buf2 + 32768` 의 메모리를 유저가
   직접 건드리지 않는다.
2. 커널의 `file_read` 가 그 주소에 바이트를 *기록* 한다. → 이게 그
   페이지에 대한 **첫 access**.
3. SPT 에 없으니 page fault. user = false (커널이 일으킴).
4. `vm_try_handle_fault` 가 §12.5.6 의 분기로 user_rsp 를 회수.
5. `addr ≥ rsp − 8` 휴리스틱 트리비얼하게 통과 (addr 는 rsp 위쪽 32KB).
6. `vm_stack_growth(addr)` 가 그 페이지 *한 장* 을 SPT 에 등록.

> **"stack growth" 가 의미하는 건 SPT 에 등록된 페이지 수가 한 장 늘었다
> 는 사건** 이지, "rsp 가 그쪽으로 새로 자랐다" 가 아니다. rsp 는 이미
> 64KB 아래에 있다. 늘어나는 건 **SPT 의 커버 범위**.

두 단어를 분리하자:
- **rsp 이동** — `sub rsp, N` 한 줄로 즉시. 메모리 미관여.
- **페이지 등록** — fault 시점에 lazy. 실제 메모리 사용.
